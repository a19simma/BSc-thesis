...
class VizDoomTrain(Env):
    ...
    def step(self, action):
        ...
        movement_reward = (self.game.make_action(actions[action], 4)) / 1000
        reward = 0 
        if self.game.get_state(): 
            state = self.game.get_state().screen_buffer
            game_variables = self.game.get_state().game_variables
            damage_taken, killcount, ammo = game_variables
            ...
            self.ammo = ammo
            damage_taken_delta = damage_taken_delta / 1000
            killcount_delta = killcount_delta / 1000
            ammo_delta = ammo_delta / 1000
            reward = movement_reward + damage_taken_delta*5 + killcount_delta*100  + ammo_delta*5
        else:
